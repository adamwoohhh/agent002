...
test("log viewer server exposes file list and file detail", async () => {
  ...
  const fileName = "agx-run-sample.jsonl";
  await writeFile(
    path.join(logsDir, fileName),
    [
      '{"sequence":0,"type":"run_started","timestamp":"2026-04-22T07:57:10.549Z","runId":"sample-run","eventId":"root"}',
  ...
  try {
    ...
    assert.equal(detailResponse.body.summary.runId, "sample-run");
