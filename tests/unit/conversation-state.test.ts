...
test("conversation state manager carries pending question and fact memory across supplement turns", async () => {
  ...
  ({ state } = await manager.beginTurn(state, "小明今天 10 岁，小明爸爸今年多少岁？", "new_request"));
...
test("conversation state manager clears pending question after final answer", async () => {
  ...
  ({ state } = await manager.beginTurn(state, "冰箱里有 3 个苹果，早上我吃了苹果，还剩下几个苹果？", "new_request"));
  state = manager.completeTurn(state, {
    status: "clarify",
    answer: "你早上吃了几个苹果？",
  });
...
test("fallbackResolveTurnMode defaults to supplement while clarification is active", () => {
  ...
  assert.equal(fallbackResolveTurnMode("新的问题是什么？", null, null), "new_request");
});
